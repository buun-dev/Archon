...
describe('executeDagWorkflow -- AI node prompt substitution failure', () => {
  ...
  it('records a node_failed event when $BASE_BRANCH cannot be resolved (not a silent skip)', async () => {
    ...
    await executeDagWorkflow(
      ...
      join(testDir, 'artifacts'),
      join(testDir, 'logs'),
      '', // base branch unresolved (container resume regression: empty on the resume path)
      'docs/',
      minimalConfig
    );

    // The substitution throw must surface as a node_failed event — the 4b9ba482
    // regression was a node_started with NO terminal event, so the run looked
    // like the node vanished while downstream all_success silently skipped.
